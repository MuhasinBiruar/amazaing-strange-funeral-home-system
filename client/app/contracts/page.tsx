...
const burialOptions: BurialOption[] = [
  { id: "modern", name: "Modern Hearse", detail: "Sleek temperature-controlled transport for modern dignity.", price: 0, icon: Truck },
  { id: "traditional", name: "Traditional Karwahe", detail: "Grand, ornate carriage with traditional processional decor.", price: 5000, icon: Landmark },
];
...
export default function ContractsPage() {
  ...
  const [signature, setSignature] = useState<SignatureRecord | null>(() => {
    ...
    try {
      const savedSignature = localStorage.getItem("villa-elisa-contract-signature");
      return savedSignature ? (JSON.parse(savedSignature) as SignatureRecord) : null;
    } catch {
      return null;
    }
  });
  ...
  const implementDetails = burialOptions.find((item) => item.id === selectedImplement) ?? burialOptions[0];
  const total = packageDetails.price + implementDetails.price;
  const isSigned = signature?.packageId === selectedPackage && signature.implementId === selectedImplement;

  useEffect(() => {
    authClient.getSession().then(({ data }) => {
      const user = data?.user;
      if (user?.name || user?.email) setSigner(user.name || user.email || "Authorized staff member");
    });
  }, []);

  function saveSelection() {
    try {
      localStorage.setItem("villa-elisa-contract-draft", JSON.stringify({ selectedPackage, selectedImplement }));
  ...
  function signAgreement() {
    ...
    setSignature(record);
    try {
      localStorage.setItem("villa-elisa-contract-signature", JSON.stringify(record));
      setStatus(`Agreement signed digitally by ${record.signer}.`);
    } catch {
      setStatus("Agreement signed for this session, but could not be persisted.");
    }
  }
  ...
  function generateContract() {
    ...
    download.href = URL.createObjectURL(new Blob([agreement], { type: "text/plain" }));
      ...
      <div className="min-h-screen bg-[#f7f8fa] text-[#122039] lg:flex">
        <aside className={`${menuOpen ? "block" : "hidden"} fixed inset-y-0 left-0 z-30 w-64 border-r border-[#e0e3e8] bg-[#f1f2f4] lg:static lg:block lg:w-52 lg:shrink-0`}>
          ...
          <nav className="space-y-1 p-4">
            {navItems.map(({ name, icon: Icon, route, active }) => <button key={name} onClick={() => router.push(route)} className={`flex w-full items-center gap-3 rounded-md px-3 py-3 text-left text-[10px] uppercase tracking-wide ${active ? "bg-white font-semibold text-[#122039] shadow-sm" : "text-gray-500 hover:bg-white/70"}`}><Icon size={14} />{name}</button>)}
          </nav>
        ...
        <div className="min-w-0 flex-1">
          ...
          <main className="mx-auto max-w-7xl px-5 py-7 sm:px-8 lg:px-12 lg:py-10">
            <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-start">
              ...
              <div className="flex items-center gap-4"><span className="text-sm text-gray-500">Drafts</span><button onClick={saveSelection} className="rounded-md bg-[#122039] px-5 py-3 text-xs font-semibold tracking-wide text-white shadow-sm hover:bg-[#1d3151]">Save Selection</button></div>
            </div>
            {status && <p role="status" className="mb-5 rounded-md border border-[#d9dde3] bg-white px-4 py-3 text-sm text-gray-600">{status}</p>}
            ...
            <section className="mt-12 grid gap-8 lg:grid-cols-[1fr_360px]">
              <div><h2 className="mb-5 font-serif text-2xl text-[#122039]">Burial Implements</h2><div className="grid gap-4 sm:grid-cols-2">{burialOptions.map(({ id, name, detail, price, icon: Icon }) => { const selected = id === selectedImplement; return <button key={id} onClick={() => setSelectedImplement(id)} className={`min-h-32 rounded-md border bg-white p-5 text-left ${selected ? "border-[#122039] shadow-sm" : "border-[#e1e4e9]"}`}><Icon size={19} className="mb-5 text-[#122039]" /><p className="font-serif text-lg">{name}</p><p className="mt-1 max-w-52 text-[10px] leading-4 text-gray-500">{detail}</p><p className="mt-2 text-xs font-semibold text-[#122039]">{price ? `+ ${money.format(price)}` : "Included"}</p></button>; })}</div></div>
              <aside className="rounded-md border border-[#d9dde3] bg-white p-6 shadow-sm"><h2 className="font-serif text-xl">Investment Summary</h2><div className="mt-6 space-y-4 text-xs"><div className="flex justify-between text-gray-500"><span>Selected Package ({packageDetails.name})</span><strong className="text-[#122039]">{money.format(packageDetails.price)}</strong></div><div className="flex justify-between border-b border-gray-200 pb-4 text-gray-500"><span>Additional Implements ({implementDetails.name})</span><strong className="text-[#122039]">{money.format(implementDetails.price)}</strong></div><div className="flex justify-between text-sm font-bold"><span>Contract Total</span><span>{money.format(total)}</span></div></div><div className="mt-5 rounded-md bg-[#eef0f2] p-4"><p className="text-[9px] font-bold uppercase tracking-wide">Required downpayment (15%)</p><p className="mt-1 font-serif text-xl">{money.format(total * 0.15)}</p><CalendarDays size={17} className="float-right -mt-5 text-gray-400" /></div></aside>
            </section>

            <section className="mt-12"><div className="mx-auto max-w-3xl bg-white px-8 py-12 shadow-md sm:px-16"><div className="text-center"><p className="font-serif text-2xl italic">Villa Elisa</p><p className="mt-1 text-[8px] tracking-[0.35em] text-gray-400">FUNERAL SERVICES & MEMORIAL CHAPELS</p><h2 className="mt-12 font-serif text-lg font-bold tracking-widest">MEMORIAL SERVICE AGREEMENT</h2></div><p className="mt-10 text-[10px] leading-5 text-gray-500">This Agreement is entered into on this ____ day of __________, {new Date().getFullYear()}, between Villa Elisa Funeral Services and the undersigned Representative.</p><h3 className="mt-8 text-[10px] font-bold">I. SCOPE OF SERVICES</h3><p className="mt-3 text-[10px] leading-5 text-gray-500">The Service Provider agrees to facilitate the “{packageDetails.name}” memorial package including {implementDetails.name.toLowerCase()}, preservation, and logistical handling of the remains.</p><h3 className="mt-7 text-[10px] font-bold">II. FINANCIAL OBLIGATIONS</h3><p className="mt-3 text-[10px] leading-5 text-gray-500">The total contract value is set at {money.format(total)}. A non-refundable downpayment of 15% ({money.format(total * 0.15)}) is required upon execution of this document.</p><div className="mt-16 grid gap-8 sm:grid-cols-2"><div className="border-t border-gray-300 pt-2 text-[9px] text-gray-400">{isSigned ? `Digitally signed by ${signature?.signer} on ${new Date(signature.signedAt).toLocaleString("en-PH")}` : "Authorized Admin Signature"}</div><button onClick={signAgreement} className={`flex items-center justify-center gap-2 border border-dashed py-3 text-[9px] font-bold tracking-wide ${isSigned ? "border-green-300 bg-green-50 text-green-700" : "border-gray-300 bg-gray-50"}`}><PenLine size={13} />{isSigned ? "SIGNED DIGITALLY" : "SIGN DIGITALLY"}</button></div></div><div className="mt-8 text-center"><button onClick={generateContract} className="inline-flex items-center gap-3 rounded-md bg-[#122039] px-10 py-4 text-xs font-bold tracking-[0.15em] text-white shadow-lg hover:bg-[#1d3151]"><ShieldCheck size={16} />GENERATE FINAL CONTRACT</button></div></section>
          </main>
        </div>
      </div>
    </PageGuard>
