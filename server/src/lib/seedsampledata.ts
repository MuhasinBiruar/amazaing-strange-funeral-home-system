...
async function seed() {
    ...
    console.log("  Created:", admin.user.id, "username: jdsantos / password: AdminPass123!");
    ...
    console.log("  Created:", staff.user.id, "username: mrreyes / password: StaffPass123!");
    ...
    const dr1 = await pool.query(
     ...
     RETURNING caseid`,
        ["Pedro", "Cruz", "Garcia", "Cardiac arrest", "Natural", "Male, 5'8\"", "pending", false, "Direct", admin.user.id]
    );
    const dr2 = await pool.query(
     ...
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9, now(), $10)
     RETURNING caseid`,
        ["Ana", "Lopez", "Torres", "Unknown", "Natural", "Female, 5'4\"", "active", true, "Life", staff.user.id]
    );
    console.log("  Created caseid:", dr1.rows[0].caseid, "and", dr2.rows[0].caseid);
